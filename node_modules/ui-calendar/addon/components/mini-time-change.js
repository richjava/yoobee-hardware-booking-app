...
export default Ember.Component.extend({
    ...
    _durationChoices: computed('durationChoices', function () {
        ...
        return typeOf(choices) !== 'array' ? false : a(choices.map(choice = > {
                return typeOf(choice) === 'object' ? choice : {title: `${choice}
        m`,
        value: Number(choice)
    }
        ;
...
:
1439, // a tick for every minute in the day
    _end
:
 ...
 */
    intervals
:
computed('_start', '_end', function () {
    const {_start, _end} = this.getProperties('_start', '_end');
    return Number(_end) - Number(_start);
}),
    midPoint
:
computed('intervals', function () {
    return this.get('intervals') / 2;
}),
    timeDefault
:
'center', // if time is not set, then event fired to move it to middle of range [left, center, right]
    ticks
:
'720', // default to a tick at 12 noon
...
minedOut: computed('_start', '_shadowTime', function () {
    ...
    maxedOut
:
computed('_end', '_shadowTime', function () {
